/**
 * Stripe purchase webhook route
 * Handles Stripe webhook events for one-time purchases (and additional event types
 * the router is subscribed to — subscription, invoice, payment_intent, customer).
 *
 ...
 * 2. Test with Stripe CLI: stripe listen --forward-to localhost:8788/api/billing/purchases/webhook
 * 3. Verify idempotency (webhooks can be delivered multiple times)
 * 4. NEVER skip signature verification
 * 5. Test both test mode and live mode scenarios
 *
 * CRITICAL PATTERNS:
 ...
 *
 * NOTE: This file uses plain Hono (not OpenAPIHono) because webhook
...
import { routeStripeEvent, extractLedgerContext } from './webhookRouter';
 ...
 * plus subscription/invoice/payment_intent/customer events routed via webhookRouter.
 */
webhookRoutes.post('/purchases/webhook', async c => {
