...
export function ScoringSummary({ checklistState, onDomainClick }: ScoringSummaryProps) {
  ...
  const getDomainChipColor = (domainKey: string) => {
    ...
    switch (judgement) {
      case 'Low':
      case 'Low (except for concerns about uncontrolled confounding)':
        baseColor = 'bg-green-100 text-green-800 border-green-300'; break;
      case 'Moderate':
        baseColor = 'bg-yellow-100 text-yellow-800 border-yellow-300'; break;
      case 'Serious':
        baseColor = 'bg-orange-100 text-orange-800 border-orange-300'; break;
      case 'Critical':
        baseColor = 'bg-red-100 text-red-800 border-red-300'; break;
      default:
  ...
  const getDomainShortName = (dk: string) => {
    const map: Record<string, string> = {
      domain1a: 'D1', domain1b: 'D1', domain2: 'D2',
      domain3: 'D3', domain4: 'D4', domain5: 'D5', domain6: 'D6',
    };
    return map[dk] || dk;
  };
