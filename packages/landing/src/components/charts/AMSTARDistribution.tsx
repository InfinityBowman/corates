...
export const AMSTARDistribution = forwardRef<SVGSVGElement, AMSTARDistributionProps>(function AMSTARDistribution(
  { data = [], width: widthProp, height: heightProp, title = 'Level Judgments Across Included Reviews', greyscale = false },
  ref,
) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (rect.width > 0 && rect.height > 0) {
        setContainerSize({ width: Math.max(rect.width, 400), height: Math.max(rect.height, 400) });
      }
    };
  ...
  useEffect(() => {
    ...
    for (let q = 0; q < nQuestions; q++) {
      const qData: any = { question: q + 1, label: `Q${q + 1}`, counts: { yes: 0, 'partial yes': 0, no: 0, 'no ma': 0 }, percentages: {} as any };
      data.forEach(study => {
    ...
    const yScale = d3.scaleBand().domain(processedData.map(d => d.label)).range([0, chartHeight]).padding(0.1);
    ...
    svg.append('text').attr('x', width / 2).attr('y', 30).attr('text-anchor', 'middle')
      .attr('font-size', titleFont + 'px').attr('font-weight', '600').attr('fill', '#111827').text(title);
      ...
      ['yes', 'partial yes', 'no ma', 'no'].forEach(category => {
        ...
        if (percent > 0 && segWidth > 0 && barHeight > 0) {
          chartGroup.append('rect')
            .attr('x', xScale(cumPercent)).attr('y', y)
            .attr('width', segWidth).attr('height', barHeight)
            .attr('fill', colors[category]).attr('stroke', '#ffffff').attr('stroke-width', 1);

          if (percent >= 5) {
            const isLightBg = greyscale ? (category === 'no' || category === 'no ma') : category === 'partial yes';
            chartGroup.append('text')
              .attr('x', xScale(cumPercent) + segWidth / 2).attr('y', (y ?? 0) + barHeight / 2)
              .attr('text-anchor', 'middle').attr('dominant-baseline', 'middle')
              .attr('font-size', '12px').attr('font-weight', '600')
              .attr('fill', isLightBg ? '#000000' : '#ffffff').text(`${percent.toFixed(1)}`);
          }
        }
        cumPercent += percent;
      });
    ...
    // Y-axis labels
    chartGroup.selectAll('.y-label').data(processedData).enter().append('text')
      .attr('x', -10).attr('y', (d: any) => (yScale(d.label) ?? 0) + yScale.bandwidth() / 2)
      .attr('text-anchor', 'end').attr('dominant-baseline', 'middle')
      .attr('font-size', '13px').attr('font-weight', '500').attr('fill', '#374151')
      .text((d: any) => d.label);

    // X-axis
    const xAxis = d3.axisBottom(xScale).tickFormat((d: any) => `${d}`).ticks(5);
    chartGroup.append('g').attr('transform', `translate(0, ${chartHeight})`).call(xAxis)
      .selectAll('text').attr('font-size', '12px').attr('fill', '#374151');

    // X-axis label
    chartGroup.append('text').attr('x', chartWidth / 2).attr('y', chartHeight + 50)
      .attr('text-anchor', 'middle').attr('font-size', '14px').attr('font-weight', '500')
      .attr('fill', '#374151').text(`Percentage of SRs (%), N=${totalStudies}`);

    // Y-axis label
    svg.append('text').attr('transform', 'rotate(-90)').attr('x', -height / 2).attr('y', 20)
      .attr('text-anchor', 'middle').attr('font-size', '14px').attr('font-weight', '500')
      .attr('fill', '#374151').text('Items of AMSTAR-2');
