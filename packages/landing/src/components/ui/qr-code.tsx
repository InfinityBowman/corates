...
function QRCodePattern({
  className,
  ...props
}: React.ComponentProps<typeof QrCodePrimitive.Pattern>) {
...
function QRCodeOverlay({
  className,
  ...props
}: React.ComponentProps<typeof QrCodePrimitive.Overlay>) {
  return (
  ...
  return (
    <QrCodePrimitive.DownloadTrigger
      className={cn(
        ...
        'hover:bg-secondary/80 focus:ring-ring focus:ring-2 focus:outline-none',
        className,
      )}
      {...props}
    />
