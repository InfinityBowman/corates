...
export function SubscriptionCard({ subscription, onManage, manageLoading }: SubscriptionCardProps) {
  ...
  const daysRemaining = useMemo(() => getDaysRemaining(sub.currentPeriodEnd), [sub.currentPeriodEnd]);
  ...
  return (
    <div className="border-border bg-card overflow-hidden rounded-xl border shadow-sm">
      {/* Header */}
      <div className="relative bg-gradient-to-r from-primary to-primary/90 px-6 py-5">
        ...
        {isTrial && daysRemaining !== null && (
          ...
          >
            <ClockIcon className={`h-4 w-4 ${daysRemaining <= 3 ? 'text-amber-200' : 'text-blue-200'}`} />
          ...
          <div className="mb-4 flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-amber-600">
                Your subscription will end on {periodEndDate}. You&apos;ll be downgraded to the Free plan.
              </p>
            </div>
          </div>
        )}
