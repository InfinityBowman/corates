...
export function useReferenceOperations() {
  ...
  const [refPdfFiles, setRefPdfFiles] = useState([]);
  ...
  const processRefPdfs = useCallback(async pdfFiles => {
    const processedPdfs = [];
  ...
  const lookupPdfsForRefs = useCallback(async refs => {
  ...
  const toggleRefSelection = useCallback(id => {
  ...
  const attachPdfToRef = useCallback((refId, pdfData, pdfFileName) => {
    setImportedRefs(prev =>
      prev.map(ref => (ref._id === refId ? { ...ref, pdfData, pdfFileName } : ref)),
    );
  }, []);

  const markPdfMatched = useCallback((pdfId, refId) => {
    setRefPdfFiles(prev =>
  ...
  const restoreState = useCallback(savedState => {
    if (savedState.importedRefs?.length > 0) setImportedRefs(savedState.importedRefs);
