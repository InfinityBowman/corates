...
const mockVerifyAuth = verifyAuth as Mock;
...
describe('UserSession Durable Object', () => {
  ...
  async function getUserSessionStub(userId: string) {
  ...
  describe('Path Extraction', () => {
    it('should extract userId from path correctly', async () => {
      const stub = await getUserSessionStub('test-user-1');
      await runInDurableObject(stub, async (instance: UserSession, _state: DurableObjectState) => {
