 ...
 */
function createDocWithStudies(studyCount: number = 3) {
  const doc = new Y.Doc();
 ...
 */
function simulateSyncProtocol(serverDoc: Y.Doc, clientDoc: Y.Doc) {
...
describe('ProjectDoc Y.js Sync Protocol', () => {
  describe('Initial sync for new users', () => {
    it('should sync all studies to a new client with empty state', () => {
      ...
      const study0 = clientStudies.get('study-0') as Y.Map<unknown>;
    ...
    it('should sync nested checklist data', () => {
      ...
      const study = studiesMap.get('study-0') as Y.Map<unknown>;

      // Add a checklist to the study
      const checklistsMap = study.get('checklists') as Y.Map<unknown>;
      ...
      const clientStudy = clientDoc.getMap('reviews').get('study-0') as Y.Map<unknown>;
      const clientChecklists = clientStudy.get('checklists') as Y.Map<unknown>;
      expect(clientChecklists.size).toBe(1);

      const clientChecklist = clientChecklists.get('checklist-1') as Y.Map<unknown>;
  ...
  describe('Incremental updates after initial sync', () => {
    ...
    it('should sync updates to existing studies', () => {
      ...
      const serverStudy = serverDoc.getMap('reviews').get('study-0') as Y.Map<unknown>;
