...
let app: Hono;
// R2 and DO mocks use `any` since the full R2Bucket/DurableObjectNamespace types are complex
let mockR2Bucket: any;
let mockProjectDO: any;
  ...
  mockR2Bucket = {
    list: async ({ prefix }: { prefix?: string }) => {
      const objects = Array.from(storedObjects.entries())
        .filter(([key]: [string, unknown]) => !prefix || key.startsWith(prefix))
        ...
        .sort((a, b) => (b.uploaded as number) - (a.uploaded as number));
    ...
    // R2 put accepts many body types; keeping `any` for the body parameter since typing
    // all possible R2 body variants is impractical in test mocks
    put: async (key: string, body: any, options: Record<string, any>) => {
...
async function fetchAvatar(path: string, init: FetchInit = {}) {
...
describe('Avatar Routes - POST /api/users/avatar', () => {
  ...
  it('should sync avatar to project memberships', async () => {
    ...
    projectDO.fetch = async (request: Request) => {
      const url = new URL(request.url);
      if (url.pathname === '/sync-member') {
