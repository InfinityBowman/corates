...
async function fetchAccountMerge(path: string, init: FetchInit = {}) {
...
describe('Account Merge Routes - POST /api/accounts/merge/initiate', () => {
  ...
  it('should enforce rate limiting', async () => {
    ...
    const originalGet = (env as any).RATE_LIMIT_KV.get;
    (env as any).RATE_LIMIT_KV.get = vi.fn(async (key: string) => {
    ...
    try {
      ...
      (env as any).RATE_LIMIT_KV.get = originalGet;
    }
  });
});
