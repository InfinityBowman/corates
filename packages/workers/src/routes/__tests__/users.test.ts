...
async function fetchUsers(path: string, init: FetchInit = {}) {
...
describe('User Routes - GET /api/users/search', () => {
  ...
  it('should mask email when query does not include @', async () => {
    ...
    const user = body.find((u: Record<string, unknown>) => u.id === user2.id);
  ...
  it('should exclude current user from results', async () => {
    ...
    const body = await json(res);
    expect(body.find((u: Record<string, unknown>) => u.id === currentUser.id)).toBeUndefined();
    expect(body.find((u: Record<string, unknown>) => u.id === otherUser.id)).toBeDefined();
  });
