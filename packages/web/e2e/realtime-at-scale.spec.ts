...
test(`Realtime reconciliation with ${STUDY_COUNT} ROB2 studies`, { timeout: 300_000 }, async ({ browser }) => {
  ...
  try {
    ...
    expect(timings.projectPageLoad, 'Project page load too slow').toBeLessThan(MAX_PROJECT_LOAD_MS);
    expect(timings.reconcilePageLoad, 'Reconcile page load too slow').toBeLessThan(MAX_RECONCILE_LOAD_MS);
