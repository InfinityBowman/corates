    ...
    <div class={compact() ? 'space-y-4' : 'space-y-6'}>
      ...
      <Show when={showHeader()}>
        <div class="flex items-center justify-between">
          <div>
            <h2 class={compact() ? 'text-lg font-semibold text-gray-900' : 'text-2xl font-bold text-gray-900'}>
              Local Appraisals
            </h2>
            <Show when={!compact()}>
      ...
      {/* Checklists Grid */}
      <div class={compact() ? 'grid gap-3 md:grid-cols-2' : 'grid gap-4 md:grid-cols-2 lg:grid-cols-3'}>
        <Show
          when={checklists().length > 0}
          fallback={
            <Show when={!loading()}>
              <div class={`col-span-full rounded-lg border-2 border-dashed border-gray-300 bg-white text-center ${compact() ? 'py-6' : 'py-12'}`}>
                <div class="mb-2 text-gray-500 text-sm">No local appraisals</div>
                <button
                  onClick={() => navigate('/checklist')}
        ...
        >
          <For each={checklists()}>
            {checklist => (
              <div class={`group relative rounded-lg border border-gray-200 bg-white shadow-sm transition-all duration-200 hover:border-gray-300 hover:shadow-md ${compact() ? 'p-4' : 'p-6'}`}>
                {/* Local badge */}
                ...
                <div class={compact() ? 'mb-3 pr-12' : 'mb-4'}>
                  <Editable
                    ...
                    variant="heading"
                    class={compact() ? 'text-base font-semibold text-gray-900' : 'text-lg font-semibold text-gray-900'}
                ...
                <div class={`flex items-center justify-between text-xs text-gray-500 ${compact() ? 'mb-3' : 'mb-4'}`}>
                  <span>
                    Updated{' '}
