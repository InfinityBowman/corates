...
import { FiArrowLeft, FiPlus, FiSearch, FiFilter, FiGrid, FiList, FiMoreHorizontal, FiExternalLink, FiUser, FiClock } from 'solid-icons/fi';
...
const mockStats = [
  { label: 'Total Studies', value: 24, change: '+3 this week', color: 'from-violet-500 to-fuchsia-500' },
...
const mockActivity = [
  { user: 'Sarah Chen', action: 'completed review', study: 'MBSR for Chronic Low Back Pain', time: '2 hours ago' },
  { user: 'Michael Torres', action: 'started review', study: 'Mindfulness Meditation for Pediatric', time: '5 hours ago' },
  { user: 'Emily Watson', action: 'requested reconciliation', study: 'MBCT on Body Awareness', time: '1 day ago' },
];
  ...
  return (
    <div class="relative h-12 w-12">
      <svg class="h-12 w-12 -rotate-90">
        <circle cx="24" cy="24" r="18" stroke="currentColor" stroke-width="3" fill="none" class="text-slate-700" />
        <circle
          cx="24"
          cy="24"
          r="18"
...
function StatusPill(props) {
  ...
  return (
    <span class={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium ${styles[props.status]}`}>
      <span class={`h-1.5 w-1.5 rounded-full ${
        props.status === 'completed' ? 'bg-emerald-400' :
        props.status === 'reconcile' ? 'bg-amber-400' :
        props.status === 'in-progress' ? 'bg-blue-400 animate-pulse' :
        'bg-slate-400'
      }`} />
      {labels[props.status]}
    </span>
...
function ConfidenceBadge(props) {
  ...
  return (
    <span class={`rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${styles[props.level]}`}>
      {props.level.replace('-', ' ')}
    </span>
  );
}
  ...
  return (
    <div class="min-h-screen bg-slate-950 text-white">
      ...
      <div class="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div class="absolute bottom-0 right-1/4 h-60 w-60 rounded-full bg-cyan-500/10 blur-[80px]" />
      </div>
      ...
      <div class="relative">
          ...
          <div class="mx-auto max-w-7xl px-6 py-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-4">
                <button class="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-800/50 text-slate-400 hover:bg-slate-800 hover:text-white transition-all">
              ...
              <div class="flex items-center gap-3">
                <button class="flex items-center gap-2 rounded-lg bg-gradient-to-r from-violet-600 to-fuchsia-600 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-violet-500/25 hover:shadow-violet-500/40 transition-all">
        ...
        <div class="mx-auto max-w-7xl px-6 py-6">
          <div class="grid grid-cols-4 gap-4">
            <For each={mockStats}>
              {stat => (
                <div class="group relative overflow-hidden rounded-2xl border border-slate-800/50 bg-slate-900/50 p-5 backdrop-blur-sm hover:border-slate-700/50 transition-all">
                  <div class={`absolute inset-0 bg-gradient-to-br ${stat.color} opacity-0 group-hover:opacity-5 transition-opacity`} />
                  <div class="relative">
                    <p class="text-sm text-slate-400">{stat.label}</p>
                    <div class="mt-2 flex items-end justify-between">
                      <span class="text-3xl font-bold tracking-tight">{stat.value}</span>
                      <Show when={stat.percentage}>
                        <span class={`text-sm font-medium bg-gradient-to-r ${stat.color} bg-clip-text text-transparent`}>
                          {stat.percentage}%
                        </span>
                      </Show>
          ...
          <div class="grid grid-cols-12 gap-6">
            ...
            <div class="col-span-8">
              ...
              <div class="mb-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <div class="relative">
                    <FiSearch class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
                    <input
                      type="text"
                      placeholder="Search studies..."
                      class="w-64 rounded-lg border border-slate-800 bg-slate-900/50 py-2 pl-10 pr-4 text-sm text-white placeholder-slate-500 focus:border-violet-500/50 focus:outline-none focus:ring-1 focus:ring-violet-500/50"
                    />
                  </div>
                  <button
              ...
              <div class="space-y-3">
                <For each={mockStudies}>
                  {study => (
                    <div class="group relative overflow-hidden rounded-xl border border-slate-800/50 bg-slate-900/30 p-4 backdrop-blur-sm hover:border-slate-700/50 hover:bg-slate-900/50 transition-all cursor-pointer">
                      <div class="flex items-start gap-4">
                        {/* Progress Ring */}
                        <Show when={study.status !== 'pending'}>
                          <ProgressRing value={
                            study.status === 'completed' ? 100 :
                            Math.round(study.reviewers.reduce((a, r) => a + r.progress, 0) / Math.max(study.reviewers.length, 1))
                          } />
                        </Show>
                        <Show when={study.status === 'pending'}>
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <h3 class="font-medium text-white truncate group-hover:text-violet-300 transition-colors">
                            {study.title}
                          </h3>
                          ...
                          <Show when={study.reviewers.length > 0}>
                            <div class="mt-3 grid grid-cols-2 gap-3">
                              <For each={study.reviewers}>
                                {reviewer => (
                                  <ReviewerProgress name={reviewer.name} progress={reviewer.progress} />
                                )}
                              </For>
                            </div>
                          </Show>
                          ...
                          <Show when={study.reviewers.length === 0}>
                            <button class="mt-3 flex items-center gap-2 text-sm text-slate-500 hover:text-violet-400 transition-colors">
                        ...
                        {/* Actions */}
                        <button class="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 opacity-0 group-hover:opacity-100 hover:bg-slate-800 hover:text-white transition-all">
                      ...
                      <Show when={study.status === 'reconcile'}>
                        <div class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-500 animate-pulse" />
                      </Show>
                    </div>
                  )}
                </For>
            ...
            <div class="col-span-4">
              <div class="rounded-2xl border border-slate-800/50 bg-slate-900/30 p-5 backdrop-blur-sm">
                      ...
                      <div class="flex gap-3">
                        ...
                        <div class="min-w-0">
                          <p class="text-sm text-slate-300">
                            <span class="font-medium text-white">{activity.user.split(' ')[0]}</span>
                            {' '}{activity.action}
                          </p>
                          <p class="text-xs text-slate-500 truncate">{activity.study}</p>
                ...
                <button class="mt-4 w-full rounded-lg border border-slate-800 py-2 text-sm text-slate-400 hover:border-slate-700 hover:text-white transition-all">
                  View all activity
                </button>
              </div>
              ...
              <div class="mt-4 rounded-2xl border border-slate-800/50 bg-slate-900/30 p-5 backdrop-blur-sm">
                ...
                <div class="space-y-2">
                  <button class="flex w-full items-center gap-3 rounded-lg border border-slate-800 p-3 text-left text-sm text-slate-300 hover:border-violet-500/30 hover:bg-violet-500/5 transition-all">
                  ...
                  <button class="flex w-full items-center gap-3 rounded-lg border border-slate-800 p-3 text-left text-sm text-slate-300 hover:border-fuchsia-500/30 hover:bg-fuchsia-500/5 transition-all">
