...
const mockProject = {
  ...
  description:
    'A systematic review examining RCTs of mindfulness interventions for chronic pain conditions including fibromyalgia, low back pain, and arthritis.',
...
function StatusBadge(props) {
  ...
  return (
    <span
      class={`inline-block px-2 py-0.5 text-[10px] font-medium tracking-widest uppercase ${colors[props.status]}`}
    >
      {labels[props.status]}
    </span>
  );
...
function RatingBadge(props) {
  ...
  return (
    <span
      class={`inline-block border px-2 py-0.5 text-[10px] font-medium tracking-widest uppercase ${colors[props.rating]}`}
    >
      {props.rating}
    </span>
  );
    ...
    <div class='min-h-screen bg-stone-50'>
      ...
      <header class='border-b-2 border-stone-900 bg-white'>
        <div class='mx-auto max-w-6xl px-8 py-6'>
          {/* Breadcrumb */}
          <div class='mb-6 flex items-center gap-2 text-xs tracking-widest text-stone-500 uppercase'>
            <button class='flex items-center gap-1 transition-colors hover:text-stone-900'>
          ...
          <div class='grid grid-cols-12 gap-8'>
            <div class='col-span-8'>
              <h1 class='font-serif text-4xl leading-tight font-bold tracking-tight text-stone-900'>
                {mockProject.name}
              </h1>
              <p class='mt-4 font-serif text-lg leading-relaxed text-stone-600'>
            ...
            <div class='col-span-4 border-l border-stone-200 pl-8'>
              <div class='space-y-4'>
                <div>
                  <h3 class='mb-2 text-[10px] font-medium tracking-widest text-stone-400 uppercase'>
                    Research Team
                  </h3>
                  <div class='space-y-1'>
                    <For each={mockProject.members}>
                      {member => (
                        <div class='flex items-center gap-2'>
                          <div class='flex h-6 w-6 items-center justify-center rounded-full bg-stone-200 text-[10px] font-medium text-stone-600'>
                            {member.name
                              .split(' ')
                              .map(n => n[0])
                              .join('')}
                          ...
                          <Show when={member.role === 'Lead'}>
                            <span class='text-[9px] tracking-widest text-stone-400 uppercase'>
                              (Lead)
                            </span>
                          </Show>
                        </div>
                ...
                <div>
                  <h3 class='mb-1 text-[10px] font-medium tracking-widest text-stone-400 uppercase'>
                    Created
                  </h3>
          ...
          <div class='flex gap-0'>
            {[
              ...
              >
                {tab.label}
                <span
                  class={`ml-2 text-xs ${activeSection() === tab.id ? 'text-stone-500' : 'text-stone-300'}`}
                >
                  {tab.count}
                </span>
                <Show when={activeSection() === tab.id}>
                  <div class='absolute right-0 bottom-0 left-0 h-0.5 bg-stone-900' />
                </Show>
              </button>
            ))}
          </div>
      ...
      <main class='mx-auto max-w-6xl px-8 py-8'>
        ...
        <div class='mb-8 flex items-center justify-between'>
          ...
          <button class='flex items-center gap-2 bg-stone-900 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-stone-800'>
              ...
              <article class='group py-6 first:pt-0'>
                <div class='grid grid-cols-12 gap-6'>
                  ...
                  <div class='col-span-8'>
                    ...
                    <h2 class='cursor-pointer font-serif text-xl leading-snug font-medium text-stone-900 transition-colors group-hover:text-stone-700'>
                      {study.title}
                    </h2>

                    <p class='mt-2 text-sm text-stone-500'>{study.authors}</p>
                  ...
                  <div class='col-span-3'>
                    <Show when={study.reviewers.length > 0}>
                      <h4 class='mb-2 text-[10px] font-medium tracking-widest text-stone-400 uppercase'>
                        Reviewers
                      </h4>
                    ...
                    <Show when={study.reviewers.length === 0}>
                      <button class='flex items-center gap-1 text-xs text-stone-400 transition-colors hover:text-stone-600'>
        ...
        <div class='mt-8 text-center'>
          <button class='text-sm font-medium text-stone-500 transition-colors hover:text-stone-900'>
            Load more studies
          </button>
        </div>
      </main>
