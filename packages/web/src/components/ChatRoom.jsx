...
export default function ChatRoom({ apiBase }) {
  ...
  const connectWebSocket = () => {
    try {
      const wsUrl = apiBase.replace('http', 'ws') + `/api/rooms/${roomId()}`;
  ...
  const loadChatHistory = async () => {
    try {
      const response = await fetch(`${apiBase}/api/rooms/${roomId()}/messages`);
      if (response.ok) {
  ...
  return (
    <div class="bg-gray-800 rounded-lg p-6">
      <h2 class="text-xl font-bold mb-4 text-blue-400">💬 Chat Room (Durable Objects + WebSocket)</h2>
