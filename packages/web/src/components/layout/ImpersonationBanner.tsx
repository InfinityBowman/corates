 ...
 * Only rendered by AppLayout when impersonation is active.
 */
...
export function ImpersonationBanner() {
  const impersonationAdminName = useAdminStore(s => s.impersonationAdminName);
    ...
    <div className='fixed inset-x-0 top-0 z-100 flex items-center justify-between bg-amber-500 px-4 py-2 text-sm font-medium text-white'>
      <span>
        ...
        {impersonationAdminName && (
          <span className='ml-1 opacity-75'>(as {impersonationAdminName})</span>
        )}
      </span>
      <Button
        size='xs'
