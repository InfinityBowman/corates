...
export default function LocalChecklistItem(props) {
  ...
  return (
    <div
      class={`group flex items-center rounded-lg transition-colors ${props.isSelected ? 'bg-blue-100 text-blue-700' : 'text-secondary-foreground hover:bg-muted'} `}
    >
      <button
        onClick={handleClick}
        class='flex min-w-0 flex-1 items-center gap-2 px-3 py-2 text-left focus:outline-none'
      >
        <HiOutlineDocumentCheck class='text-muted-foreground h-4 w-4 shrink-0' />
      ...
      <button
        ...
        class='text-muted-foreground/70 mr-1 rounded p-1.5 opacity-0 transition-colors group-hover:opacity-100 hover:bg-red-50 hover:text-red-600 focus:opacity-100 focus:ring-2 focus:ring-blue-500 focus:outline-none'
