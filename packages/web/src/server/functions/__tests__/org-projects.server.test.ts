...
describe('updateProjectById', () => {
  ...
  it('returns 403 for non-project-members in same org', async () => {
    ...
    try {
      await updateProjectById(mockSession(), createDb(env.DB), org.id, project.id, {
...
describe('deleteProjectById', () => {
  it('allows owner to delete project', async () => {
    ...
    const result = await deleteProjectById(mockSession(), createDb(env.DB), org.id, project.id);

    expect(result.success).toBe(true);
    expect(result.deleted).toBe(project.id);
