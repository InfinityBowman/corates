 ...
 */
export async function extractPdfTitle(pdfData: ArrayBuffer): Promise<string | null> {
 ...
 */
async function extractPdfTitleInternal(pdfData: ArrayBuffer): Promise<string | null> {
 ...
 */
export async function extractPdfDoi(pdfData: ArrayBuffer): Promise<string | null> {
 ...
 */
async function extractPdfDoiInternal(pdfData: ArrayBuffer): Promise<string | null> {
  const engine = await initEmbedPdfEngine();
  ...
  try {
    ...
    if (metadata?.custom) {
      for (const [key, value] of Object.entries(metadata.custom as Record<string, unknown>)) {
