...
function CompleteProfilePage() {
    ...
    if (hasEditedName || hasAutofilledName || !user) {
      ...
      if (!user && !hasEditedName && !hasAutofilledName) {
        ...
        if (!firstName.trim() && !lastName.trim() && pendingName) {
          setFirstName(pendingName); // eslint-disable-line react-hooks/set-state-in-effect -- one-time localStorage consumption
  ...
  useEffect(() => {
    ...
    if (pendingPersona) {
      setPersona(pendingPersona); // eslint-disable-line react-hooks/set-state-in-effect -- one-time localStorage consumption
