...
The database schema is defined in `packages/db/src/schema.ts` using Drizzle's SQLite schema builder.
...
Always create DB client from the Worker env binding:
...
DrizzleKit generates migrations (migrations/0000_*.sql, 0001_*.sql, etc.)
    ↓
Test SQL Constant (packages/web/src/__tests__/server/migration-sql.js) [generated]
```

**Key Principle:** Only the Drizzle schema (`packages/db/src/schema.ts`) needs to be maintained manually. Migrations are automatically generated by DrizzleKit.

### Files Overview

#### Source Files (Maintained Manually)

- **`packages/db/src/schema.ts`** - Drizzle ORM schema definitions
...
- **`packages/web/src/__tests__/server/migration-sql.js`** - Test SQL constant
...
Edit `packages/db/src/schema.ts` to add/modify tables, columns, or constraints:
...
- Reads the Drizzle schema from `packages/db/src/schema.ts`
...
**When to run:** After making any changes to `packages/db/src/schema.ts`
...
- Generates `packages/web/src/__tests__/server/migration-sql.js` with the SQL as an exported constant
...
- Validate inputs with Zod schemas (`packages/web/src/__tests__/server/seed-schemas.ts`)
...
- Always update the schema first - Edit `packages/db/src/schema.ts` before generating migrations
...
1. Review `packages/db/src/schema.ts` for the intended schema
...
Check the Zod schema in `packages/web/src/__tests__/server/seed-schemas.ts`:
...
- `packages/web/src/__tests__/server/helpers.ts` - Test utilities and seed functions
- `packages/web/src/__tests__/server/seed-schemas.ts` - Zod validation schemas for seed functions
- `packages/web/src/__tests__/server/migration-sql.js` - Generated test SQL constant (do not edit)
