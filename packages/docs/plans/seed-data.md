...
Else: await import('./routes/dev/index.js') and delegate.
2) Seed runner (Node) using Faker + PDF upload
...
Uploads PDFs by sending multipart/form-data or raw application/pdf bodies to a companion dev endpoint (or same endpoint) so the worker can R2.put(...).
3) Reset-all behavior (your choice)
...
ProjectDoc DO: add an internal-only DO endpoint to clear yjs-state (or overwrite with an empty doc) for seeded projects.
4) Creating real login-capable users
...
Set one user to role='admin' so the admin dashboard features are available.
5) Subscriptions/grants and project scenarios
...
free tier
Projects per org: 0–3, with varied membership (owner-only, 2–4 members, invited-but-not-accepted).
6) Seeding Yjs snapshots into ProjectDoc
