...
**Leverage existing infrastructure:** `packages/workers/src/durable-objects/EmailQueue.js`

CoRATES already has a robust EmailQueue Durable Object with:

- Automatic retry with exponential backoff (configurable via `EMAIL_RETRY_CONFIG`)
- Dead letter queue for failed emails
- Alarm-based background processing
- Postmark integration via `createEmailService`

**New File:** `packages/workers/src/routes/billing/handlers/dunning.js`
...
export async function queueDunningEmail(params, ctx) {
  const { subscriptionId, orgId, userEmail, userName, invoiceUrl, amountDue, currency, attemptCount } = params;
  ...
  // Queue via EmailQueue DO (existing infrastructure)
...
\${attemptCount >= 3 ? 'FINAL NOTICE: Your subscription will be canceled if payment is not received.\n\n' : ''}
Please update your payment method: \${invoiceUrl}
