/**
 * ROBINS-I V2 Checklist Schema
 *
 * Risk Of Bias In Non-randomized Studies - of Interventions, Version 2
 */

import { JUDGEMENTS, OVERALL_DISPLAY } from './scoring.js';

// Response option types used across different questions
export const RESPONSE_TYPES = {
  YN: ['Y', 'N'] as const,
  STANDARD: ['Y', 'PY', 'PN', 'N'] as const,
  WITH_NI: ['Y', 'PY', 'PN', 'N', 'NI'] as const,
  WITH_NA: ['NA', 'Y', 'PY', 'PN', 'NI'] as const,
  WITH_NA_FULL: ['NA', 'Y', 'PY', 'PN', 'N', 'NI'] as const,
  WITH_NA_NO_NI: ['NA', 'Y', 'PY', 'PN', 'N'] as const,
  WEAK_STRONG_NO: ['Y', 'PY', 'WN', 'SN', 'NI'] as const,
  WITH_NA_WEAK_STRONG_NO: ['NA', 'Y', 'PY', 'WN', 'SN', 'NI'] as const,
  WEAK_STRONG_YES: ['SY', 'WY', 'PN', 'N', 'NI'] as const,
  WITH_NA_WEAK_STRONG_YES: ['NA', 'SY', 'WY', 'PN', 'N', 'NI'] as const,
} as const;
...
export interface ROBINSQuestion {
  id: string;
  number?: string;
  text: string;
...
export interface ROBINSDomain {
  id: string;
  name: string;
  subtitle?: string;
  questions?: Record<string, ROBINSQuestion>;
  subsections?: Record<
    string,
    {
      name: string;
...
export const SECTION_B: Record<string, ROBINSQuestion> = {
  b1: {
    id: 'b1',
...
// Domain definitions
export const DOMAIN_1A: ROBINSDomain = {
  id: 'domain1a',
...
export const DOMAIN_1B: ROBINSDomain = {
  id: 'domain1b',
  name: 'Domain 1: Bias due to confounding',
  subtitle:
...
export const DOMAIN_2: ROBINSDomain = {
  id: 'domain2',
...
export const DOMAIN_3: ROBINSDomain = {
  id: 'domain3',
...
export const DOMAIN_4: ROBINSDomain = {
  id: 'domain4',
...
export const DOMAIN_5: ROBINSDomain = {
  id: 'domain5',
...
export const DOMAIN_6: ROBINSDomain = {
  id: 'domain6',
...
export function getActiveDomainKeys(isPerProtocol: boolean): DomainKey[] {
  const base: DomainKey[] = ['domain2', 'domain3', 'domain4', 'domain5', 'domain6'];
  return isPerProtocol ? ['domain1b', ...base] : ['domain1a', ...base];
}

// Get all questions for a domain (flattened)
export function getDomainQuestions(domainKey: string): Record<string, ROBINSQuestion> {
  const domain = ROBINS_I_CHECKLIST[domainKey as keyof typeof ROBINS_I_CHECKLIST];
  if (!domain || domainKey === 'sectionB') return {};

  const typedDomain = domain as ROBINSDomain;
  if (typedDomain.subsections) {
    let allQuestions: Record<string, ROBINSQuestion> = {};
    Object.values(typedDomain.subsections).forEach(subsection => {
      allQuestions = { ...allQuestions, ...subsection.questions };
    });
    return allQuestions;
  }

  return typedDomain.questions || {};
}
