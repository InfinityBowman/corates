...
const betterAuthCache = new Map<string, string>();
let cacheTime = 0;
...
export function registerBetterAuthTools(server: McpServerType): void {
  server.tool(
    'better_auth_docs',
    ...
    async ({
      ...
      try {
        ...
        return {
          content: [{ type: 'text', text: `Error fetching Better Auth docs: ${errorMessage}` }],
        };
      }
    },
  );
