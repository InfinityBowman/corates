...
interface DrizzleDocsCache {
  header: string;
  sections: Map<string, string>;
}

// Cache for parsed Drizzle docs
let drizzleDocsCache: DrizzleDocsCache | null = null;
 ...
 */
async function fetchDrizzleDocs(): Promise<DrizzleDocsCache> {
  const now = Date.now();
 ...
 */
function generateDrizzleIndex(header: string, sections: Map<string, string>): string {
...
export function registerDrizzleTools(server: McpServerType): void {
  server.tool(
    'drizzle_docs',
    ...
    async ({ path: docPath }): Promise<{ content: Array<{ type: 'text'; text: string }> }> => {
      try {
        ...
        return {
          content: [{ type: 'text', text: `Error fetching Drizzle docs: ${errorMessage}` }],
        };
      }
    },
  );
