...
function assertSafeDocName(doc: unknown): asserts doc is string {
...
async function listLocalDocsWithLlmsTxt(docsRoot: string): Promise<DocInfo[]> {
  try {
...
export function registerLocalDocsTools(server: McpServerType, docsRoot: string): void {
  server.tool(
    'docs_list',
  ...
  server.tool(
    ...
    async ({ doc }): Promise<{ content: Array<{ type: 'text'; text: string }> }> => {
