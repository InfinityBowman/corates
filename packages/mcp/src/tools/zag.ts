...
interface ZagDocsCache {
  sections: Map<string, string>;
}

// Cache for parsed Zag docs
let zagDocsCache: ZagDocsCache | null = null;
let zagCacheTime = 0;
 ...
 */
async function fetchZagDocs(): Promise<ZagDocsCache> {
  const now = Date.now();
 ...
 */
function generateZagIndex(sections: Map<string, string>): string {
...
export function registerZagTools(server: McpServerType): void {
  server.tool(
    'zag_docs',
    ...
    async ({
      ...
      try {
        ...
        return {
          content: [{ type: 'text', text: `Error fetching Zag docs: ${errorMessage}` }],
        };
      }
    },
  );
