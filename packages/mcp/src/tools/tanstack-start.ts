...
async function loadManifest(): Promise<DocsManifest> {
  if (!manifestCache) {
    const manifestPath = path.join(__dirname, '..', '..', 'tanstack-start-manifest.json');
    try {
      const content = await fs.readFile(manifestPath, 'utf8');
      manifestCache = JSON.parse(content) as DocsManifest;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new Error(`Failed to load start manifest: ${errorMessage}`);
    }
  }
...
async function fetchWithCache(url: string): Promise<string> {
  ...
  if (startCache.has(url)) {
    return startCache.get(url)!;
  }
...
function generateIndex(manifest: DocsManifest, framework: string): string {
  ...
  let index = `# TanStack Start Documentation Index (${framework})\n\n`;
...
export function registerTanStackStartTools(server: McpServerType): void {
  server.tool(
    'tanstack_start_docs',
    'Fetch TanStack Start documentation from GitHub. Returns an index of all topics when no path is provided, or specific documentation when a path is given. Defaults to Solid framework.',
    {
      path: z
        .string()
        .describe(
    ...
    async ({
      ...
      try {
        ...
        const url = `${START_BASE_URL}/${framework}/${matchedPath}.md`;
        ...
        return {
          content: [
            {
              type: 'text',
              text: `# TanStack Start: ${matchedPath} (${framework})\n\n${text}`,
            },
          ],
        };
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        return {
          content: [{ type: 'text', text: `Error fetching TanStack Start docs: ${errorMessage}` }],
        };
      }
    },
  );
