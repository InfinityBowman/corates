...
function createProjectStore() {
    ...
    try {
      if (projects && Array.isArray(projects) && userId) {
        ...
        localStorage.setItem(
          PROJECT_LIST_CACHE_TIMESTAMP_KEY,
          Date.now().toString(),
        )
  ...
  const initialProjectList = cachedData?.projects
    ? {
    ...
    setStore(
      produce((s) => {
        ...
        if (data.members !== undefined)
          s.projects[projectId].members = data.members
  ...
  function addProjectToList(project) {
    ...
    saveCachedProjectList(
      store.projectList.items,
      store.projectList.cachedUserId,
    )
  }
  ...
  function removeProjectFromList(projectId) {
    setStore(
      produce((s) => {
        s.projectList.items = s.projectList.items.filter(
          (p) => p.id !== projectId,
        )
      }),
    )
  ...
  async function fetchProjectList(userId, options = {}) {
    ...
    const shouldForceRefresh =
      force || (cachedUserId && cachedUserId !== userId)
