...
export function createPdfActions(
        ...
        try {
          ...
          if (refData) {
            ...
            if (refData.publicationYear)
              metadata.publicationYear = refData.publicationYear
  ...
  async function view(studyId, pdf) {
    ...
    try {
      ...
      if (!data) {
        ...
        await cachePdf(projectId, studyId, pdf.fileName, data).catch(
          console.warn,
        )
      }
  ...
  async function upload(studyId, file, tag = 'secondary') {
    ...
    try {
      ...
      cachePdf(projectId, studyId, uploadResult.fileName, arrayBuffer).catch(
        (err) => console.warn('Failed to cache PDF:', err),
      )
      ...
      if (uploadResult?.fileName) {
        deletePdf(projectId, studyId, uploadResult.fileName).catch(
          (cleanupErr) =>
