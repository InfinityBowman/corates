...
import { config, urls } from '@lib/landing/config'
...
function getPrice(plan: (typeof BASE_PLANS)[0], billing: string) {
  if (plan.monthlyAmount === null) return { amount: null, unit: '' }

  if (billing === 'yearly') {
    const monthlyEquivalent = Math.round(
      plan.monthlyAmount * (1 - ANNUAL_DISCOUNT),
    )
...
export const Route = createFileRoute('/pricing')({
  ...
  head: () => {
    ...
    const description =
      'Simple plans for evidence synthesis teams. Start free, upgrade anytime.'
    return {
      title,
      meta: [
...
function Pricing() {
  ...
  const plans = () =>
    BASE_PLANS.map((plan) => ({ ...plan, price: getPrice(plan, billing()) }))
      ...
      <main class="mx-auto max-w-6xl px-6 py-12">
        <div class="mx-auto max-w-2xl text-center">
          ...
          <p class="mt-4 text-lg text-gray-600">
            CoRATES is currently in active development and testing. We're
            working closely with early users to refine features, workflows, and
            team-based collaboration. Pricing will be announced closer to launch
            and will reflect the needs of individual researchers, teams, and
            institutions. Interested in getting involved?{' '}
              ...
              >
                ...
                <div class="mb-6">
                  <Show
                    ...
                    fallback={
                      <div class="text-2xl font-bold text-gray-900">Custom</div>
                    }
                  >
