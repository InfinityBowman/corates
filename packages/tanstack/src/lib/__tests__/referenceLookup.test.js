  ...
  parseFromString(xmlText) {
    ...
    const mockDoc = {
      querySelector: vi.fn((selector) => {
        ...
        if (selector === 'ArticleTitle') {
          if (xmlText.includes('<ArticleTitle>')) {
...
describe('parseIdentifiers', () => {
  ...
  it('should parse comma-separated identifiers', () => {
    const result = parseIdentifiers(
      '10.1234/first, 10.1234/second, 10.1234/third',
    )
    expect(result).toEqual(['10.1234/first', '10.1234/second', '10.1234/third'])
  })
      ...
      json: () =>
        Promise.resolve({
          oa_locations: [
            {
              url_for_pdf:
                'https://europepmc.org/articles/PMC12345/pdf/article.pdf',
      ...
      json: () =>
        Promise.resolve({
          oa_locations: [
            {
              url_for_pdf:
                'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC12345/pdf/article.pdf',
              host_type: 'publisher',
            },
          ],
        }),
...
describe('fetchFromDOI', () => {
  ...
  it('should throw for 404 (not found)', async () => {
    ...
    await expect(fetchFromDOI('10.1234/notfound')).rejects.toThrow(
      'DOI not found',
    )
  })
  ...
  it('should throw for other HTTP errors', async () => {
    ...
    await expect(fetchFromDOI('10.1234/test')).rejects.toThrow(
      'Failed to fetch DOI: 500',
    )
  })
...
describe('fetchReferenceByIdentifier', () => {
  ...
  it('should detect DOI with https prefix', async () => {
    ...
    const result = await fetchReferenceByIdentifier(
  ...
  it('should throw for unrecognized identifier', async () => {
    ...
    await expect(
      fetchReferenceByIdentifier('invalid-identifier'),
    ).rejects.toThrow('Could not identify reference')
  })
