...
export function calculateInterRaterReliability(studies, getChecklistData) {
  ...
  for (const study of dualReviewerStudies) {
    ...
    const reviewer1Checklist = completedChecklists.find(
      (c) => c.assignedTo === study.reviewer1,
    )
    const reviewer2Checklist = completedChecklists.find(
      (c) => c.assignedTo === study.reviewer2,
    )
