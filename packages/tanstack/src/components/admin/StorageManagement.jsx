...
export default function StorageManagement() {
  ...
  const toggleSelectAll = () => {
    ...
    const allCurrentPageSelected =
      docs.length > 0 && docs.every((doc) => selected.has(doc.key))
        ...
        <div class="mx-auto max-w-7xl p-6">
          ...
          <div class="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
            <p class="text-sm text-blue-800">
              <strong>Note:</strong> This dashboard shows all PDFs in R2
              storage. PDFs marked as "Orphaned" have a project ID that no
              longer exists in the database (e.g., from failed cleanup when
              projects were deleted). You can safely delete orphaned PDFs to
                      ...
                      >
                        <Show
                          when={(() => {
                            ...
                            return (
                              docs.length > 0 &&
                              docs.every((doc) => selected.has(doc.key))
                            )
                          })()}
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <Show when={documentsData()?.truncated}>
                    <p class="text-xs text-orange-600">
                      Results truncated after processing 10,000 objects. Use
                      pagination to continue.
                    </p>
                  </Show>
                </div>
        ...
        >
          <div class="space-y-4">
            <p class="text-sm text-gray-600">
              Are you sure you want to delete {deleteDialog()?.length || 0}{' '}
              document
              {deleteDialog()?.length === 1 ? '' : 's'}? This action cannot be
              undone.
            </p>
