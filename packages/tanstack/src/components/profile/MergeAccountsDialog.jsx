...
export default function MergeAccountsDialog(props) {
  ...
  async function handleSendCode() {
    ...
    try {
      ...
      const displayValue =
        isOrcid && result.targetOrcidId
          ? result.targetOrcidId
          : result.targetEmail
      showToast.success(
        'Code Sent',
        `A verification code has been sent to ${displayValue}`,
      )
    } catch (err) {
  ...
  async function handleCompleteMerge() {
    ...
    try {
      ...
      const linkedInfo = result.mergedProviders.length
        ? `Linked: ${result.mergedProviders.join(', ')}`
        : ''
      showToast.success(
        'Accounts Merged',
        `Successfully merged accounts. ${linkedInfo}`,
      )
      props.onSuccess?.()
    } catch (err) {
        ...
        <Show when={step() === STEPS.PROMPT}>
          <div class="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3">
            ...
            <div class="text-sm text-amber-800">
              ...
              <p class="mt-1">
                If you own both accounts and want to combine them, you can merge
                them into one.
              </p>
            </div>
          </div>
          ...
          <p class="text-sm text-gray-600">
            Merging will combine all your projects, data, and sign-in methods
            into this account. The other account will be deleted.
          </p>
        ...
        <Show when={step() === STEPS.ENTER_CODE}>
          ...
          <div class="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 disabled:opacity-50"
            >
              {loading() ? <FiLoader class="h-4 w-4 animate-spin" /> : null}
              Verify Code
            </button>
          </div>
        </Show>
        ...
        <Show when={step() === STEPS.CONFIRM}>
          ...
          <div class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
            ...
            <p class="mt-1">
              ...
              ) will be deleted and all its data merged into your current
              account.
            </p>
          </div>
