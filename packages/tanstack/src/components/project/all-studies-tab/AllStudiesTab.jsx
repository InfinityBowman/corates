...
export default function AllStudiesTab() {
  ...
  const [googleDriveTargetStudyId, setGoogleDriveTargetStudyId] =
    createSignal(null)
  ...
  onMount(async () => {
    ...
    if (
      restoreParams?.type === 'addStudies' &&
      restoreParams.projectId === projectId
    ) {
      try {
    ...
    <div>
      ...
      <div class="mb-2 flex items-center justify-between">
        <p class="text-sm text-gray-500">
          {studies().length} {studies().length === 1 ? 'study' : 'studies'} in
          this project
        </p>
      </div>
