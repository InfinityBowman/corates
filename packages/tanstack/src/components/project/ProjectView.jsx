...
import { useParams, useNavigate, useLocation } from '@tanstack/solid-router'
...
export default function ProjectView() {
  ...
  const connectionState = () =>
    projectStore.getConnectionState(params.projectId)
  ...
  const [pendingPdfs, setPendingPdfs] = createSignal(
    pendingData?.pendingPdfs || null,
  )
  const [pendingRefs, setPendingRefs] = createSignal(
    pendingData?.pendingRefs || null,
  )
  const [pendingDriveFiles, setPendingDriveFiles] = createSignal(
    pendingData?.driveFiles || null,
  )
        ...
        uploadPdf(params.projectId, studyId, arrayBuffer, pdf.fileName)
          .then((result) => {
            ...
            try {
              ...
              deletePdf(params.projectId, studyId, result.fileName).catch(
                console.warn,
              )
            }
          })
  ...
  createEffect(() => {
    ...
    for (const file of driveFiles) {
      ...
      const metadata = {
        ...(file.metadata || {}),
        importSource:
          file.metadata?.importSource || file.importSource || 'google-drive',
      }
      const studyId = projectActionsStore.study.create(
        title,
        abstract,
        ...
        importFromGoogleDrive(file.id, params.projectId, studyId)
          .then((result) => {
            try {
              ...
              console.error('Failed to add PDF metadata:', metaErr)
              // Clean up orphaned file
              deletePdf(params.projectId, studyId, result.file.fileName).catch(
                console.warn,
              )
            }
          })
          .catch((err) =>
            console.error('Error importing Google Drive file:', err),
          )
      }
    }
  })
  ...
  const tabDefinitions = [
    {
      value: 'overview',
      label: 'Overview',
      icon: <BiRegularHome class="h-4 w-4" />,
    },
    {
      value: 'all-studies',
      label: 'All Studies',
  ...
  const tabFromUrl = () => {
    ...
    return tabDefinitions.map((tab) => tab.value).includes(tab)
      ? tab
      : 'overview'
  }
  ...
  const handleTabChange = (value) => {
    ...
    navigate({ to: `${location.pathname}${queryString}`, replace: true })
  }
      ...
      <ProjectProvider projectId={params.projectId}>
        <ProjectHeader
          ...
          onUpdateDescription={(desc) =>
            projectActionsStore.project.updateDescription(desc)
          }
