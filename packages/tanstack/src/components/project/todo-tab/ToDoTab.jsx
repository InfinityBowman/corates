...
export default function ToDoTab() {
  ...
  const handleCreateChecklist = async (studyId, type, assigneeId) => {
    ...
    try {
      const success = projectActionsStore.checklist.create(
        studyId,
        type,
        assigneeId,
      )
  ...
  const openChecklist = (studyId, checklistId) => {
    navigate({
      to: `/projects/${projectId}/studies/${studyId}/checklists/${checklistId}?tab=todo`,
    })
  }
          ...
          <Show when={hasData()}>
            <div class="py-16 text-center">
              ...
              <p class="mx-auto max-w-md text-gray-500">
                Studies assigned to you will appear here. Complete your
                appraisals to move them to the next stage.
              </p>
            </div>
          </Show>
        }
          ...
          {(study) => (
            <TodoStudyRow
              ...
              onToggleChecklistForm={() =>
                setShowChecklistForm((prev) =>
                  prev === study.id ? null : study.id,
                )
              }
              ...
              onOpenChecklist={(checklistId) =>
                openChecklist(study.id, checklistId)
              }
