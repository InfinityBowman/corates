    ...
    for (const study of studiesList) {
      ...
      if (study.reconciliation?.reconciledChecklistId) {
        const reconciledChecklist = study.checklists.find(
          (c) =>
            c.id === study.reconciliation.reconciledChecklistId &&
            c.type === 'AMSTAR2',
        )
        if (
          reconciledChecklist &&
      ...
      if (!checklistToScore) {
        checklistToScore = study.checklists.find(
          (c) =>
            c.type === 'AMSTAR2' && c.status === CHECKLIST_STATUS.COMPLETED,
        )
      }
      ...
      const fullChecklist = props.getChecklistData?.(
        study.id,
        checklistToScore.id,
      )
      ...
      fallback={
        <div class="rounded-lg border border-gray-200 bg-white py-8 text-center">
          <p class="text-gray-500">
            No completed AMSTAR 2 appraisals to display.
          </p>
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
              ...
              <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
                ...
                <div class="text-center">
                  ...
                  <p class="text-xs text-gray-500">
                    ({summaryData().counts['Critically Low']} of{' '}
                    {summaryData().total})
                  </p>
                </div>
              </div>
