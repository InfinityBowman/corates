...
export default function PreviousReviewersView(props) {
  ...
  const originalChecklists = createMemo(() => {
    ...
    return getOriginalReviewerChecklists(
      props.study,
      props.reconciliationProgress,
    )
  })
  ...
  createEffect(() => {
    ...
    try {
      ...
      const data1 = checklists[0]
        ? getChecklistData(props.study.id, checklists[0].id)
        : null
      const data2 = checklists[1]
        ? getChecklistData(props.study.id, checklists[1].id)
        : null
