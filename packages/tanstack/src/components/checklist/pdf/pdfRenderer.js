...
export function createPdfRenderer(document, scrollHandler) {
  ...
  createEffect(() => {
    ...
    if (doc && prevScale !== null && prevScale !== currentScale) {
      ...
      const visiblePages = scrollHandler.getVisiblePages
        ? scrollHandler.getVisiblePages()
        : new Set()
  ...
  async function renderWithPriority(pages, scaleValue) {
    ...
    const visiblePages = scrollHandler.getVisiblePages
      ? scrollHandler.getVisiblePages()
      : new Set()
    const currentPageNum = scrollHandler.currentPage
      ? scrollHandler.currentPage()
      : 1
    ...
    for (let i = 0; i < prioritized.length; i += RENDER_CONCURRENCY) {
      ...
      await Promise.allSettled(
        batch.map((pageNum) => renderPage(pageNum, scaleValue)),
      )
    }
  }
    ...
    try {
      ...
      if (textLayerDiv) {
        try {
          ...
          const selectionLayer = createTextSelectionLayer(
            textLayerDiv,
            page,
            viewport,
          )
