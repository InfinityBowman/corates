...
export default function Navbar(props) {
  return (
    <nav
      class="flex flex-wrap gap-1 py-1 pl-1"
      aria-label="Question navigation"
    >
      <For each={props.store.questionKeys}>
        {(key, index) => (
          <QuestionPill key={key} questionIndex={index()} store={props.store} />
        )}
      </For>
...
function QuestionPill(props) {
  ...
  const isCurrentPage = () =>
    props.store.viewMode === 'questions' &&
    props.store.currentPage === props.questionIndex

  const isAgreement = () =>
    props.store.comparisonByQuestion[key()]?.isAgreement ?? true
...
function SummaryButton(props) {
  ...
  return (
    <Tooltip
      content="View summary of all questions"
      placement="bottom"
      openDelay={200}
    >
      <button
...
function ResetButton(props) {
  return (
    <Tooltip
      content="Reset all answers to unresolved"
      placement="bottom"
      openDelay={200}
    >
      <button
