...
export default function ReconciliationWrapper() {
  ...
  const [reconciledChecklistLoading, setReconciledChecklistLoading] =
    createSignal(false)
  const [hasCheckedForReconciled, setHasCheckedForReconciled] =
    createSignal(false)

  // Get or create reconciled checklist (with race condition prevention)
  createEffect(() => {
    const state = connectionState()
    if (!state.synced || reconciledChecklistId() || hasCheckedForReconciled())
      return
    ...
    if (
      existingReconciled &&
      existingReconciled.status !== CHECKLIST_STATUS.COMPLETED
    ) {
    ...
    const newChecklistId = createProjectChecklist(
      params.studyId,
      checklistType,
      null,
    )
