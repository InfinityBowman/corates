...
function createBetterAuthStore() {
  ...
  const authChannel =
    typeof BroadcastChannel !== 'undefined'
      ? new BroadcastChannel('corates-auth')
      : null
        ...
        try {
          ...
          if (currentUser?.id) {
            ...
            try {
              ...
              console.warn(
                '[auth] Failed to refresh project list after visibility change:',
                err,
              )
            }
          } else {
  ...
  async function authFetch(url, options = {}) {
    try {
      ...
      if (!response.ok) {
        ...
        const { parseApiError, getErrorMessage } =
          await import('@/lib/error-utils.js')
        const parsedError = await parseApiError(response)

        if (
          parsedError.code === 'AUTH_REQUIRED' ||
          parsedError.code === 'AUTH_EXPIRED'
        ) {
      ...
      const { parseError, getErrorMessage } =
        await import('@/lib/error-utils.js')
